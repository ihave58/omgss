<?php
    ...
    if ($method == 'POST') {
        ...
        $cartid = (isset($_POST['cartid'])) ? runUserInputSanitizationHook($_POST['cartid']) : '';
        ...
        if ($prdid) {
            if ($userid) {
                $sqlfindcart = "SELECT * FROM `005_omgss_cart` WHERE `prdid`='$prdid' AND (`ip`='$ip' OR `userid`='$userid')";
            } else {
                $sqlfindcart = "SELECT * FROM `005_omgss_cart` WHERE `prdid`='$prdid' AND `ip`='$ip'";
            }
            ...
            if (mysqli_num_rows($resfindcart) > 0) {
                ...
                $sql = "DELETE FROM `005_omgss_cart` WHERE `id`='$foundcartid'";
                ...
                $response = array('status' => 'error', 'message' => 'Product not in Cart.');
                echo json_encode($response);
            }


        } else if ($cartid) {
            $sqlwish = "SELECT * FROM `005_omgss_cart` WHERE `id`='$cartid'";
            ...
            if (mysqli_num_rows($reswish) > 0) {
                ...
                if (($rowwish['userid'] == $userid) || (($rowwish['userid'] == "") && ($rowwish['ip'] == $ip))) {
                    $sql = "DELETE FROM `005_omgss_cart` WHERE `id`='$cartid'";
                    if ($conn->query($sql) === TRUE) {
                        $response = array('status' => 'success', 'message' => 'Cart Item deleted Successfully');
                        echo json_encode($response);
                    } else {
                    ...
                    $response = array('status' => 'error', 'message' => 'You are not authorized to delete this cart entry.');
                    echo json_encode($response);
                }
            } else {
                $response = array('status' => 'error', 'message' => 'Invalid Cart Entry.');
                echo json_encode($response);
            }
        }
